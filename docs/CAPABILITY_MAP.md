...
design patterns, self-healing locators, and a multi-agent test-generation orchestrator — and to
ignore the 1-hour framing. This map reflects the extended scope. See `SPEC.md` §Boundaries and
the README "Design decisions" section for how that gap is framed for reviewers.

| Module id | Responsibility | Depends on |
|---|---|---|
| `core-framework` | SOLID/OOP TS architecture: `BaseElement` + typed element classes (Button, Input, Dropdown, Checkbox…) carrying a locator *and* a human-readable description; `BasePage` + page-object factory; `ApiClient` abstraction; config/env loading; Playwright fixtures | — |
| `ui-e2e-tests` | Saucedemo: log in → add item to cart → verify cart, built on `core-framework` POM | `core-framework` |
| `api-tests` | jsonplaceholder: GET a resource, assert response contract/schema | `core-framework` |
...
| `mcp-dev-tooling` | `.mcp.json` wiring the official Playwright MCP server for editor/agent-assisted authoring & debugging. Dev-time only — not called at test runtime | `core-framework` |
| `test-orchestrator-agents` | CLI (`npm run generate:test -- "<scenario>"`) driving a Planner → Writer → PageObject agent pipeline (Pipeline/Chain of Responsibility) that turns a plain-English scenario into a new spec file + page object following the framework's conventions. Reuses the same `LlmProvider` abstraction as the self-healing agent | `core-framework`, `self-healing-agent` |
...
**Build order:**
`core-framework` → (`ui-e2e-tests`, `api-tests`, `mcp-dev-tooling`, `reporting` in parallel) →
`self-healing-agent` → `test-orchestrator-agents` → `ci` → `docs` →
